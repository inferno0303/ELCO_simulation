...
class WorkloadPrioritySchedulingWF:
    ...
    def try_assign(self, func: FunctionTask, sec: SECServer) -> bool:
        ...
        cr_ik = RATIO * S_k * sqrt_nc / (self.sec_total_workload_factor[sec.id] + sqrt_nc)
        # if cr_ik < 128 * RATIO:
        #     return False
        ...
        self.sec_total_workload_factor[sec.id] += math.sqrt(func.invocations * func.workload)
        ...
        for func in self.ss.get_function_list():
            ...
            if self.sp.strategy[func.id]['offloading'] == 0:
                ...
                # 策略3：协作 SEC
                    ...
                    sqrt_nc = math.sqrt(func.invocations * func.workload)
                    cr_ik = RATIO * S_k * sqrt_nc / self.sec_total_workload_factor[target_sec.id]
